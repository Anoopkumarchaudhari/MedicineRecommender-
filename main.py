...
@app.route('/')
def index():
    return  render_template('index.html',combine=combine)
...
def predict():
    if request.method=='POST':
        ...
        if not symptoms or symptoms.strip().lower() == "symptoms":
            message = "Please enter at least one valid symptom, comma-separated."
        ...
        if unknown_symptoms:
            ...
            my_die= ast.literal_eval(die[0])

            # combine = (symtoms['Symptom_1']+", " +symtoms['Symptom_2']+", " +symtoms['Symptom_3']+", " +symtoms['Symptom_4'])
            # combine = combine.dropna().unique()

            return render_template('index.html',user_symptoms=user_symptoms,combine =combine ,pred_disease=pred_disease,desc=desc,my_med=my_med,my_die=my_die,wrkout=wrkout,my_pre=my_pre)
